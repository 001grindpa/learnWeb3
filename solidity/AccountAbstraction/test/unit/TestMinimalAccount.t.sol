...
contract TestMinimalAccount is Test {
    ...
    function testOwnerCanMintTokenToSmartWalletAndTransferItOut() public mintMockERC20ToMinimalAccountWallet {
        ...
        console.log("minimal account new token balalnce after transfer: ", usdc.balanceOf(address(minimalAccount))/1e18);
    }
    ...
    function testRecoverSignedOp() public {
        ...
        uint256 ethToSend = 0;
        bytes memory mintCallData = abi.encodeWithSelector(ERC20Mock.mint.selector, address(minimalAccount), TEST_AMOUNT);

        PackedUserOperation memory packedUserOp = sendPackedUserOp.generateSignedUserOperation(mintCallData, config);
